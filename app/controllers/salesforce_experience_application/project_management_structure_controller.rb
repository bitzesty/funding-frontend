class SalesforceExperienceApplication::ProjectManagementStructureController < ApplicationController
  ...
  def update
    ...
    # Form submitted to save and continue.  Validate and act accordingly.
      ...
      clear_pts_answers_json_for_key(:project_management_structure_question)
  ...
  # queries params and deletes by blob id
    ...
    clear_pts_answers_json_for_key(:project_management_structure_question) if
      @salesforce_experience_application.project_management_structure_files.empty?
  ...
  private
    ...
    def validate_and_store_answers(params)
      ...
      logger.info(
				'Save and continue clicked on upload a project management ' \
          'structure for case ID:' \
					  "#{@salesforce_experience_application.salesforce_case_id}"
			)

			@salesforce_experience_application.validate_project_management_structure_question = true

			@salesforce_experience_application.project_management_structure_question = 
				params[:sfx_pts_payment].nil? ? nil : \
          params[:sfx_pts_payment][:project_management_structure_question]

			# validate for files if applicant indicates they will upload some.
			@salesforce_experience_application.validate_project_management_structure_files = 
				@salesforce_experience_application.project_management_structure_question == 
          t('salesforce_experience_application.project_management_structure.bullets.' \
            'i_will_upload')
    ...
    def validate_and_store_files(params)
      ...
      logger.info(
        'Updating project_management_structure_files for salesforce case ID:' \
        "#{@salesforce_experience_application.salesforce_case_id}"
      )

      if save_files()
        logger.info(
          'Finished updating project_management_structure_files for ' \
      ...
      @salesforce_experience_application.project_management_structure_question = 
        t('salesforce_experience_application.' \
          'project_management_structure.bullets.i_will_upload')
  ...
  # Saves files.  If new files are being added, then appends those.
	def save_files()
		...
		new_files =
			params[:sfx_pts_payment][:project_management_structure_files]
		...
		@salesforce_experience_application.project_management_structure_files = 
      new_files
  ...
  def initialise_view_attributes
    ...
    @salesforce_experience_application.errors.any? do |error|
      select_file_upload_bullet = error.attribute == :project_management_structure_files
    end
    ...
    if select_file_upload_bullet
      ...
      @salesforce_experience_application.project_management_structure_question = 
        @salesforce_experience_application.pts_answers_json[
          :project_management_structure_question.to_s
        ]
  ...
  def get_attachments 
    @salesforce_experience_application.project_management_structure_files.blobs
  end
  ...
  def clear_all_attachments
    @salesforce_experience_application.project_management_structure_files.purge
  end
