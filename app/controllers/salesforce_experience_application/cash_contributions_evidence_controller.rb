class SalesforceExperienceApplication::CashContributionsEvidenceController < ApplicationController
  ...
  private
    ...
    def validate_and_store_answers(params)
      ...
      logger.info(
				'Save and continue clicked on evidence of secured cash ' \
          'contributions for case ID:' \
					  "#{@salesforce_experience_application.salesforce_case_id}"
			)

			@salesforce_experience_application.validate_cash_contributions_evidence_question = true

			@salesforce_experience_application.cash_contributions_evidence_question = 
				params[:sfx_pts_payment].nil? ? nil : \
          params[:sfx_pts_payment][:cash_contributions_evidence_question]
    ...
    def validate_and_store_files(params)
      ...
      logger.info(
        'Updating cash_contributions_evidence_files for salesforce case ID:' \
        "#{@salesforce_experience_application.salesforce_case_id}"
      )

      if save_files()
        logger.info(
          'Finished updating cash_contributions_evidence_files for ' \
  ...
  # Saves files.  If new files are being added, then appends those.
	def save_files()
		...
		new_files =
			params[:sfx_pts_payment][:cash_contributions_evidence_files]
		...
		@salesforce_experience_application.cash_contributions_evidence_files = 
      new_files
  ...
  def get_attachments 
    @salesforce_experience_application.cash_contributions_evidence_files.blobs
  end
  ...
  def clear_all_attachments
    @salesforce_experience_application.cash_contributions_evidence_files.purge
  end
